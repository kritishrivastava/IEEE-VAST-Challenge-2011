...
file_image_vastopolis = 'HW7/static/Vastopolis_Map_new.png'
...
# file_image_vastopolis = './static/Vastopolis_Map_new.png'
...
def plot_word_cloud(relevant_words):
    global source_cloud_1, word_cloud_stream_1
    ...
    #word_cloud_stream = create_word_cloud_plot()

    if len(relevant_words) <= 0:
        print("No similar words found")
        word_cloud_stream = create_word_cloud_plot("Related Search Terms")
    else:
        now = datetime.now()
        temp = now.strftime("%M_%S")
        file_wordcloud_image = 'HW7/static/word_cloud_' + temp + '.bmp'
        wordcloud = WordCloud(max_words=25, background_color='white').generate_from_frequencies(relevant_words)
        image = wordcloud.to_image()

        image.save(file_wordcloud_image)

        word_cloud_stream_1.image_url(url=[file_wordcloud_image], x=0, y=1, w=1, h=1)

    layout.children[2].children[1] = word_cloud_stream_1

def plot_event_cloud(relevant_words):
    global source_cloud_2, word_cloud_stream_2
    ...
    #word_cloud_stream_2 = create_word_cloud_plot()

    if len(relevant_words) <= 0:
        print("No words found")
        word_cloud_stream_2 = create_word_cloud_plot("Events")
    else:
        now = datetime.now()
        temp = now.strftime("%M_%S")
        file_wordcloud_event_image = 'HW7/static/word_cloud_event_'+ temp + '.bmp'

        wordcloud = WordCloud(max_words=25, background_color='white').generate_from_frequencies(relevant_words)
        image = wordcloud.to_image()

        image.save(file_wordcloud_event_image)

        word_cloud_stream_2.image_url(url=[file_wordcloud_event_image], x=0, y=1, w=1, h=1)

    layout.children[2].children[2] = word_cloud_stream_2
...
def bt_compare_click():
    ...
    relevant_tweet.loc[list(idx), 'Color'] = new_col
...
def create_word_cloud_plot(title):
    word_cloud_plot = figure(x_range=(0, 1), y_range=(0, 1.2), plot_width=300, plot_height=250,
                                 tools=[BoxZoomTool(), ResetTool()], title=title)
    word_cloud_plot.toolbar.logo = None
    word_cloud_plot.axis.visible = False
    word_cloud_plot.xgrid.grid_line_color = None
    word_cloud_plot.ygrid.grid_line_color = None
...
search_1 = TextInput(value=default_search_1, title="Enter keywords to search on:")
...
hover2 = HoverTool(tooltips=[
    ("Microblog", "@tweet_text")
])

# init figure
plot_tweet = figure(title="Microblogs plot", toolbar_location="above",
...
word_cloud_stream_1 = create_word_cloud_plot('Related search terms')
...
layout = layout(children=[[wgt_search, date_range_slider, wgt_but],
                          [plot_tweet, wgt_media_1],
